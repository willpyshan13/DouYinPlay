...
object ImageTool {
    ...
    fun dp2px(dpValue: Float): Int {
        return dp2px(BaseApplication.sInstance, dpValue)
    }
    ...
    fun px2dp(pxValue: Float): Int {
        return px2dp(BaseApplication.sInstance, pxValue)
    }
    ...
    fun sp2px(spValue: Float): Int {
        val fontScale = BaseApplication.sInstance.resources.displayMetrics.scaledDensity
    ...
    fun getBitmap(resId: Int, maxWidth: Int, maxHeight: Int): Bitmap? {
        val options = BitmapFactory.Options()
        options.inJustDecodeBounds = true
        val `is` = BaseApplication.sInstance.resources.openRawResource(resId)
        BitmapFactory.decodeStream(`is`, null, options)
        options.inSampleSize = calculateInSampleSize(options, maxWidth, maxHeight)
        options.inJustDecodeBounds = false
        return BitmapFactory.decodeStream(`is`, null, options)
