package com.will.habit.utils.image;
...
/**
 * Desc:可选择部分圆角，分为左上角、左下角、右上角、右下角
 * <p>
 * Date: 2019-11-15
...
public class RoundedCornersTransform implements Transformation<Bitmap> {
    ...
    public Resource<Bitmap> transform(@NonNull Context context, @NonNull Resource<Bitmap> resource, int outWidth, int outHeight) {
        ...
        Bitmap outBitmap = getBitmap(outHeight, source, finalWidth, finalHeight);

        return BitmapResource.obtain(outBitmap, this.mBitmapPool);
    }

    @NotNull
    private Bitmap getBitmap(float outHeight, Bitmap source, int finalWidth, int finalHeight) {
        //修正圆角
        this.radius *= (float) finalHeight / outHeight;
